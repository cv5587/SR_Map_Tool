...
bool AABB::overlaps(const AABB aabb, bool touchIsOverlap) const
{
    ...
    if (touchIsOverlap)
    {
        if (aabb._upperBound.x < _lowerBound.x || aabb._lowerBound.x > _upperBound.x ||
            aabb._upperBound.y < _lowerBound.y || aabb._lowerBound.y > _upperBound.y ||
            aabb._upperBound.z < _lowerBound.z || aabb._lowerBound.z > _upperBound.z)
        {
            rv = false;
        }
    }
...
Tree::Tree(double skinThickness, const std::vector<bool> periodicity, const D3DXVECTOR3 boxSize, bool touchIsOverlap)
    :_skinThickness(skinThickness),
    _periodicity(periodicity), _boxSize(boxSize),
    _touchIsOverlap(touchIsOverlap)
{
    ...
    // Check periodicity.
    _isPeriodic = false;
    _posMinImage.resize(3);
    _negMinImage.resize(3);
    for (unsigned int i = 0; i < 3; i++)
    {
        _posMinImage[i] = 0.5 * boxSize[i];
        _negMinImage[i] = -0.5 * boxSize[i];
...
void Tree::setBoxSize(const _vec3 boxSize)
{
    _boxSize = boxSize;
}
...
std::vector<unsigned int> Tree::query(unsigned int particle, const AABB aabb)
{
    ...
    while (stack.size() > 0)
    {
        ...
        if (_isPeriodic)
        {
            ...
            separation[0] = nodeAABB._centre.x - aabb._centre.x;
            separation[1] = nodeAABB._centre.y - aabb._centre.y;
            separation[2] = nodeAABB._centre.z - aabb._centre.z;
            ...
            if (isShifted)
            {
                nodeAABB._lowerBound.x += shift[0];
                nodeAABB._upperBound.x += shift[0];

                nodeAABB._lowerBound.y += shift[1];
                nodeAABB._upperBound.y += shift[1];

                nodeAABB._lowerBound.z += shift[2];
                nodeAABB._upperBound.z += shift[2];
            }
        }
...
{
    return _nodes[_particleMap[particle]]._aabb;
}
...
bool Tree::minimumImage(std::vector<double>& separation, std::vector<double>& shift)
{
    ...
    if (separation[0] < _negMinImage[0])
    {
        separation[0] += _periodicity[0] * _boxSize.x;
        shift[0] = _periodicity[0] * _boxSize.x;
        isShifted = true;
    }
    else
    {
        if (separation[0] >= _posMinImage[0])
        {
            separation[0] -= _periodicity[0] * _boxSize.x;
            shift[0] = -_periodicity[0] * _boxSize.x;
            isShifted = true;
    ...
    if (separation[1] < _negMinImage[1])
    {
        separation[1] += _periodicity[1] * _boxSize.y;
        shift[1] = _periodicity[1] * _boxSize.y;
        isShifted = true;
    }
    else
    {
        if (separation[1] >= _posMinImage[1])
        {
            separation[1] -= _periodicity[1] * _boxSize.y;
            shift[1] = -_periodicity[1] * _boxSize.y;
            isShifted = true;
    ...
    if (separation[2] < _negMinImage[2])
    {
        separation[2] += _periodicity[2] * _boxSize.z;
        shift[2] = _periodicity[2] * _boxSize.z;
        isShifted = true;
    }
    else
    {
        if (separation[2] >= _posMinImage[2])
        {
            separation[2] -= _periodicity[2] * _boxSize.z;
            shift[2] = -_periodicity[2] * _boxSize.z;
            isShifted = true;
