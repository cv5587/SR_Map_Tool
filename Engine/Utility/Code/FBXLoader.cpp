...
HRESULT CFBXLoader::LoadFbx(std::wstring pPath, std::wstring pDPath)
{
	for (m_iFileIndex; m_iFileIndex < 2; m_iFileIndex++)
	{
		// 파일 데이터 로드
		if (0== m_iFileIndex)
		{
		...
		else
		{
			Import(pDPath);
			// 로드된 데이터 파싱 (Mesh/Material/Skin)
			ParseNode(m_pDScene->GetRootNode());
		}
	}
...
void CFBXLoader::Import(const wstring& path)
{
	...
	else
	{
		...
		// FBX SDK 관리자 객체 생성
		m_pDManager = FbxManager::Create();

		// IOSettings 객체 생성 및 설정
		FbxIOSettings* settings = FbxIOSettings::Create(m_pDManager, IOSROOT);
		m_pDManager->SetIOSettings(settings);

		// FbxImporter 객체 생성
		m_pDScene = FbxScene::Create(m_pDManager, "");

		m_pImporter = fbxsdk::FbxImporter::Create(m_pDManager, "");

		string strPath = ws2s(path);
		m_pImporter->Initialize(strPath.c_str(), -1, m_pImporter->GetIOSettings());

		m_pImporter->Import(m_pDScene);

		m_pDScene->GetGlobalSettings().SetAxisSystem(FbxAxisSystem::DirectX);

		// 씬 내에서 삼각형화 할 수 있는 모든 노드를 삼각형화 시킨다.
		FbxGeometryConverter geometryConverter(m_pDManager);
		geometryConverter.Triangulate(m_pDScene, true);
...
void CFBXLoader::LoadMesh(FbxMesh* mesh)
{
	...
	{
		m_vDMesh.push_back(FbxMeshInfo());
		FbxMeshInfo& meshInfo = m_vDMesh.back();
...
void CFBXLoader::GetNormal(FbxMesh* mesh, FbxMeshInfo* container, _int idx, _int vertexCounter)
{
	...
	container->vertices[idx].vNormal.y = static_cast<float>(vec.mData[2]);
	container->vertices[idx].vNormal.z = static_cast<float>(vec.mData[1]);
}
