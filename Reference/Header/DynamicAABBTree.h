...
public:
    ...
    Tree(double skinThickness, const std::vector<bool> periodicity,
        const D3DXVECTOR3 boxSize, bool touchIsOverlap = true);
...
private:
    ...
    std::vector<double> _negMinImage;

    /// The position of the positive minimum image.
    std::vector<double> _posMinImage;
    ...
    bool minimumImage(std::vector<double>& separation, std::vector<double>& shift);
};
