...
Engine::_int CShambler::Update_GameObject(const _float& fTimeDelta)
{
	...
	////버그 있으면 수정요청 바람..
	//if (m_fFrame > _float(m_iMaxFrame) && m_eMiniBossState != MINIBOSS_DEAD)
	//	m_fFrame = 0.f;
	...
	//		}
	//		Engine::Get_SoundMgr()->Play_Sound(szSoundFile, MONSTERSOUND, 0.5f);
...
//	{
//		//총알 데미지 가져와서 체력깎음
//		//dynamic_cast<CBullet*>(pGameObject)->Get_
//		m_bHit = true;
//	}
//	break;
//	case OT_MONSTER:
...
HRESULT CShambler::Add_Component()
{
	...
	//Textures
	//Normal
	pComponent = m_pTextureCom = dynamic_cast<CTexture*>(Engine::Clone_Proto(L"Proto_ShamblerNormal_IdleTexture"));
	NULL_CHECK_RETURN(pComponent, E_FAIL);
...
void CShambler::Shambler_Take_Dmg(const _float& fTimeDelta)
{
	...
	//		Engine::Get_Layer(L"GameLogic")->Add_GameObject(L"ShamblerEffect", pGameObject);
	//	}
